...
_lr_productions = [
  ...
  ('start -> select','start',1,'p_start','yacc.py',7),
  ('start -> insert','start',1,'p_start','yacc.py',8),
  ('start -> update','start',1,'p_start','yacc.py',9),
  ('start -> delete','start',1,'p_start','yacc.py',10),
  ('empty -> <empty>','empty',0,'p_empty','yacc.py',15),
  ('select -> SELECT distinct select_columns FROM DATASOURCE into where order limit SIMICOLON','select',10,'p_select','yacc.py',29),
  ('insert -> INSERT INTO DATASOURCE icolumn VALUES insert_values SIMICOLON','insert',7,'p_insert','yacc.py',104),
  ('update -> UPDATE DATASOURCE SET assigns where SIMICOLON','update',6,'p_update','yacc.py',122),
  ('delete -> DELETE FROM DATASOURCE where','delete',4,'p_delete','yacc.py',132),
  ('logical -> EQUAL','logical',1,'p_logical','yacc.py',142),
  ('logical -> NOTEQUAL','logical',1,'p_logical','yacc.py',143),
  ('logical -> BIGGER_EQUAL','logical',1,'p_logical','yacc.py',144),
  ('logical -> BIGGER','logical',1,'p_logical','yacc.py',145),
  ('logical -> SMALLER_EQUAL','logical',1,'p_logical','yacc.py',146),
  ('logical -> SMALLER','logical',1,'p_logical','yacc.py',147),
  ('where -> WHERE conditions','where',2,'p_where','yacc.py',157),
  ('where -> empty','where',1,'p_where_empty','yacc.py',162),
  ('conditions -> LPAREN conditions RPAREN','conditions',3,'p_cond_parens','yacc.py',167),
  ('conditions -> conditions AND conditions','conditions',3,'p_cond_3','yacc.py',172),
  ('conditions -> conditions OR conditions','conditions',3,'p_cond_3','yacc.py',173),
  ('conditions -> exp LIKE STRING','conditions',3,'p_cond_3','yacc.py',174),
  ('conditions -> exp logical exp','conditions',3,'p_cond_3','yacc.py',175),
  ('conditions -> NOT conditions','conditions',2,'p_conditions_not','yacc.py',180),
  ('exp -> STRING','exp',1,'p_exp','yacc.py',190),
  ('exp -> COLNAME','exp',1,'p_exp','yacc.py',191),
  ('exp -> NUMBER','exp',1,'p_exp','yacc.py',192),
  ('distinct -> DISTINCT','distinct',1,'p_distinct','yacc.py',202),
  ('distinct -> empty','distinct',1,'p_distinct_empty','yacc.py',207),
  ('column -> COLNUMBER','column',1,'p_column','yacc.py',215),
  ('column -> COLNAME','column',1,'p_column','yacc.py',216),
  ('columns -> columns COMMA columns','columns',3,'p_columns','yacc.py',221),
  ('columns -> column','columns',1,'p_columns_base','yacc.py',228),
  ('select_columns -> TIMES','select_columns',1,'p_select_columns_all','yacc.py',238),
  ('select_columns -> columns','select_columns',1,'p_select_columns','yacc.py',243),
  ('into -> INTO DATASOURCE','into',2,'p_into','yacc.py',253),
  ('into -> empty','into',1,'p_into_empty','yacc.py',258),
  ('order -> ORDER BY column way','order',4,'p_order_by','yacc.py',268),
  ('order -> empty','order',1,'p_order_empty','yacc.py',273),
  ('way -> ASC','way',1,'p_way_asc','yacc.py',278),
  ('way -> empty','way',1,'p_way_asc','yacc.py',279),
  ('way -> DESC','way',1,'p_way_desc','yacc.py',284),
  ('limit -> LIMIT NUMBER','limit',2,'p_limit','yacc.py',294),
  ('limit -> empty','limit',1,'p_limit_empty','yacc.py',302),
  ('value -> STRING','value',1,'p_value','yacc.py',312),
  ('value -> NUMBER','value',1,'p_value','yacc.py',313),
  ('values -> values COMMA values','values',3,'p_values','yacc.py',318),
  ('values -> value','values',1,'p_values_end','yacc.py',330),
  ('single_values -> LPAREN values RPAREN','single_values',3,'p_single_values','yacc.py',335),
  ('insert_values -> insert_values COMMA insert_values','insert_values',3,'p_insert_values','yacc.py',340),
  ('insert_values -> single_values','insert_values',1,'p_insert_values_end','yacc.py',347),
  ('icolumn -> LPAREN columns RPAREN','icolumn',3,'p_icolumn','yacc.py',357),
  ('icolumn -> empty','icolumn',1,'p_icolumn_empty','yacc.py',362),
  ('assign -> column EQUAL value','assign',3,'p_assign','yacc.py',372),
  ('assigns -> assign COMMA assigns','assigns',3,'p_assigns','yacc.py',377),
  ('assigns -> assign','assigns',1,'p_assigns_end','yacc.py',382),
]
